...
class VariationalPeakModelChannels():
    def __init__(self,
                 data_path,
                 test_celltypes,
                 matrix,
        ...
        input_shapes, output_shape, self.train_iter = generator_to_tf_dataset_channels(load_data_channels(self.data[Dataset.TRAIN],  
                                                self.eval_cell_types,
                                                self.eval_cell_types,
                                                matrix,
                                                assaymap,
        ...
        _, _,            self.valid_iter = generator_to_tf_dataset_channels(load_data_channels(self.data[Dataset.VALID], 
                                                self.eval_cell_types,
                                                self.eval_cell_types,
                                                matrix,
                                                assaymap,
        ...
        if len(self.test_celltypes) > 0:
            _, _,            self.test_iter = generator_to_tf_dataset_channels(load_data_channels(self.data[Dataset.TEST], 
                                                   self.test_celltypes, 
                                                   self.eval_cell_types,
                                                   matrix,
                                                   assaymap,
    ...
    def eval_vector(self, data, vector, indices):
        ...
        input_shapes, output_shape, ds = generator_to_tf_dataset_channels(load_data_channels(data, 
                 self.test_celltypes,   # used for labels. Should be all for train/eval and subset for test
                 self.eval_cell_types,   # used for rotating features. Should be all - test for train/eval
                 self.matrix,
                 self.assaymap,
...
class VLPC(VariationalPeakModelChannels):
    def __init__(self,
             *args,
             **kwargs):
        ...
        if "checkpoint" in kwargs.keys():
            fileObject = open(kwargs["checkpoint"] + "/model_params.pickle" ,'rb')
            metadata = pickle.load(fileObject)
            VariationalPeakModelChannels.__init__(self, kwargs["data_path"], **metadata)
            self.model = tf.keras.models.load_model(kwargs["checkpoint"])
            
        else: 
            VariationalPeakModelChannels.__init__(self, *args, **kwargs)
