...
class ModelsTest(EpitomeTestCase):
    ...
    def test_train_early_stop_model(self):
        ...
        assert(len(train_valid_losses) == 1)
        assert(num_steps == 1)
        
        best_model_steps, num_steps, train_valid_losses = model.train(train_iters)
        results2 = model.test(self.validation_size)
        # Make sure that the model doesn't train for the full train_iters
        assert(len(train_valid_losses) < 6)
        assert(num_steps < train_iters)
        assert(best_model_steps < train_iters)
